...
| IP-TPI-4 | active | Validate physics/visual agreement. **Regenerated `small_uas_ksba_flight` loads cleanly** in `live_sim` (8543 tiles, footprint record validates across all 7 LODs, no parse/validation error). Final gate is the **in-engine** check (user): AGL reads zero when the aircraft is *visually on* the runway, not ~50 ft above. | IP-TPI-1, IP-TPI-3 | [terrain.md §Internal Spatial Index](../design/terrain.md) |
