...
| IP-CRB-11 | done | **On-ground yaw / weathervane: gear-aero moment balance (OQ-AC-5, Alt 3).** Emulate the crosswind aero yaw moment via the FBW `A_y`-hold loop; estimate the nose-gear steering **moment capability** (normal load × geometry, no modeled deflection); react the aero moment through the gear (tyre side forces + steering capability) up to that limit, yawing on the excess; **scale by contact/normal force** for a smooth flight↔ground transition. Reconcile the OQ-AC-2 azimuth gate (ground- vs aero-speed) so a parked crosswind holds a **static β = wind angle** (gear-reacted, no free weathervane). Verify: light wind → gear holds (static β, no weathervane); strong wind → slips; smooth transition as contact force fades; airborne weathervane recovered. **Done.** Math developed in [ground_directional_dynamics.md](../algorithms/ground_directional_dynamics.md): the FBW **enforces the commanded `Ny` with a contact-scaled steering control moment** (authority `N_steer,max = steering_authority_m · F_z,contact`, OQ-AC-8 — a control moment, NOT gear friction / differential braking / nose-wheel deflection) against the aero weathervane `k_a·abs(c)` (`k_a = abs(Cy_β)·qS·x_acy`, OQ-AC-7; c = crab). This conditions **which velocity the heading slaves to** via `w_hold = min(1, N_steer,max/(k_a·abs(c)))`, realized as the attitude reference `v_att,ref = v_g − (1−w_hold)·wind` (the sole change from the OQ-AC-4 aero reference). New config `x_acy_m`, `steering_authority_m`. Impl: `LandingGear` friction reaction removed; `Aircraft::gearHoldFraction` + `w_hold` at v_att_base; JSON/proto serialize. **Verified:** crosswind rollout holds the heading firmly (was spiraling to −80°/−26 m; now locks and rolls straight, ~5 m linear crab drift); reduces to coordinated flight at liftoff. TDD `GearHoldFraction_RegimeBehaviour`; 613/618 (0 regressions). **NB (dead ends):** the `a_y`-side-force-gating sketch did nothing (`β≈0` for `Ny=0`); the passive tyre-stiffness `k_g` model only partially held (short main-gear arm) — the hold is an active FBW authority limit, not a spring. | IP-CRB-10 | [aircraft.md §On-Ground Gear-Aero Yaw Balance](../design/aircraft.md), [ground_directional_dynamics.md](../algorithms/ground_directional_dynamics.md), [OQ-AC-5](../design/aircraft.md), [OQ-AC-7](../design/aircraft.md), [OQ-AC-8](../design/aircraft.md) |
