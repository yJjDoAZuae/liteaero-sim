...
| IP-CRB-11 | todo | **On-ground yaw / weathervane: gear-aero moment balance (OQ-AC-5, Alt 3).** Emulate the crosswind aero yaw moment via the FBW `A_y`-hold loop; estimate the nose-gear steering **moment capability** (normal load × geometry, no modeled deflection); react the aero moment through the gear (tyre side forces + steering capability) up to that limit, yawing on the excess; **scale by contact/normal force** for a smooth flight↔ground transition. Reconcile the OQ-AC-2 azimuth gate (ground- vs aero-speed) so a parked crosswind holds a **static β = wind angle** (gear-reacted, no free weathervane). Verify: light wind → gear holds (static β, no weathervane); strong wind → slips; smooth transition as contact force fades; airborne weathervane recovered. Design fully resolved: `N_aero = F_aero,y·x_acy` (OQ-AC-7, Alt 1) reacted by `N_gear,max = N_main + μ_y·F_z,nose·x_nose` (OQ-AC-8, Alt 1); new config `x_acy` (aerodynamic side-force lever). Authority is [aircraft.md §On-Ground Gear-Aero Yaw Balance](../design/aircraft.md); implement the balance as a contact-scaled gate on the velocity-azimuth slew. | IP-CRB-10 | [aircraft.md §On-Ground Gear-Aero Yaw Balance](../design/aircraft.md), [OQ-AC-5](../design/aircraft.md), [OQ-AC-7](../design/aircraft.md), [OQ-AC-8](../design/aircraft.md) |
