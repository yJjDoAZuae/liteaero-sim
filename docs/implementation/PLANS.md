...
| --- | --- | --- |
| [aircraft_ground_attitude.md](aircraft_ground_attitude.md) | OQ-AC-9 ground-start attitude fix: Alt 2 re-anchor `stepQnw` to `q_nw`'s own forward axis (self-correcting velocity slaving; IP-AGA-2/3 `todo`) + Alt 1 ground-trim attitude reference (init + `V→0` relaxation convergence; IP-AGA-4/5/7 `todo`) with the relaxation params in config (IP-AGA-6 `todo`). Debug accessors IP-AGA-1 `done`. OQ-AC-9 & OQ-AC-10 resolved. Strict-TDD / no-regression above very low speed | Active |
...
| [gear_collider_validation.md](gear_collider_validation.md) | Gear & collider validation capability (roadmap item 7, expanded): bind verbose per-wheel `ContactDiag` (F_x/F_y/slip/etc.) + roll-rate state + a collider-only accessor; full lateral-asymmetry scenario coverage in `gear_terrain_validation.ipynb` (banked, crabbed, banked+crabbed, single-main, wingtip, hard touchdown) in separate figures; per-wheel friction + collider-involvement traces; pytest scenario tests | Draft |
...
| [terrain_grid_overlay.md](terrain_grid_overlay.md) | Conformal terrain reference grid: north/east-aligned lines at equal metric spacing from the region centroid, drawn as a term in `terrain_grade.gdshader` (not baked, conformal at every LOD). Runtime-configurable spacing/color/opacity/physical-line-width via `configs/viewer.json`. Design authority: [`godot_plugin.md` §Conformal Terrain Grid](../design/godot_plugin.md). | Active |
