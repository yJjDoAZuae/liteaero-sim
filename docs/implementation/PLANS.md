...
| [terrain_physics_index.md](terrain_physics_index.md) | Physics `TerrainMesh` index consistency with per-LOD footprints (OQ-T-1, OQ-T-3 → Alt 4) — fixes the ~15 m `elevation_m` / AGL / collider ground-height defect. **All OQs resolved; unblocked.** Operative fix (IP-TPI-1: derive grid side from tile bounds — no re-tile, all construction paths) + validated-record layer (IP-TPI-2/3: store the array in every format, fail loudly on geometry mismatch) + validation (IP-TPI-4). OQ-T-2 slant-range/`LodSelector` deletion excluded (separate follow-up) | Active |
