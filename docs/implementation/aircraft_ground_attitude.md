...
| IP-AGA-2 | todo | Add failing pytest ground-rest pitch-up repro to `python/test/test_aircraft_bindings.py`: rest→throttle roll on `FlatTerrain`; assert bounded pitch and `qnw_ref_desync_rad ≈ 0` via `step_diag()` (TDD guard) | IP-AGA-1 | [aircraft.md §OQ-AC-9](../design/aircraft.md), [§Velocity-Slaved Attitude](../design/aircraft.md) |
...
| IP-AGA-5 | todo | Initialize `q_nw` / `att_ref_prev` to the gear-trim attitude for at-rest ground starts (weight-on-wheels and `V ≈ 0`); test that the static rest state has `α = 0`, `Δθ = 0` (no residual tilt) and does not diverge on throttle-up | IP-AGA-3, IP-AGA-4 | [aircraft.md §Ground-Trim Attitude](../design/aircraft.md) |
