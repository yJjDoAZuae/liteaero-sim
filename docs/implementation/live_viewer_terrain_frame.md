...
| IP-LV-5 | ready | **Per-LOD footprint tiling + screen-space-error thresholds.** OQ-LS-22 resolved to **Alternative 3 (per-LOD footprints)**, which **supersedes the uniform-footprint tiling first implemented here** (the uniform `_cell_side_deg` / `_effective_spacing_deg` grid remains as an interim baseline). Per [terrain_lod_rendering.md](../design/terrain_lod_rendering.md) with the adopted parameters (OQ-LR-2 → $\tau=1$ px, $\gamma=0.25$; $H_\text{ref}=1080$, $\phi_\text{ref}=90°$, $\delta=0.15$): compute per-LOD adequacy ranges $R_\ell = \varepsilon_\ell H_\text{ref}/(2\tau\tan(\phi/2))$ and per-LOD footprints $f_\ell \approx \tfrac{\gamma}{\sqrt2}(R_{\ell+1}-R_\ell)$ from the simplification error schedule; tile each LOD on its **own** grid at $f_\ell$ (small fine tiles, large coarse tiles, ≈ 6× fewer tiles than uniform, no redundant coarse geometry); record the parameters in the descriptor (so the viewer can recompute bands); export region-wide per LOD, then re-tile `small_uas_ksba_flight`. | IP-LV-4, IP-LV-8, IP-LV-9 | [terrain_lod_rendering.md](../design/terrain_lod_rendering.md), [screen_space_lod_selection.md](../algorithms/screen_space_lod_selection.md), [live_sim_view.md OQ-LS-22](../design/live_sim_view.md) |
...
| IP-LV-10 | ready | **Adopt screen-space-error thresholds in the viewer (runtime bands).** Replace the convention visibility bands ([`TerrainLoader.gd`](../../godot/addons/liteaero_sim/TerrainLoader.gd) `_LOD_VIS_BEGIN_M`/`_LOD_VIS_END_M`, from the superseded 300 m rule) with SSE adequacy ranges. Per OQ-LR-1 → Alternative 2, **recompute** $R_\ell = \varepsilon_\ell H/(2\tau\tan(\phi/2))$ each frame (or on viewport/FOV change) from the **live** viewport height $H$ and FOV $\phi$ (parameters read from the descriptor), set each node band to $[R_\ell(1-\delta)-h_\ell,\ R_{\ell+1}(1+\delta)+h_\ell]$ with crossfade $B_\ell = 2\delta R_\ell$, and drive `R_fetch`/`R_unload` from the same live $R_\ell$. Baked footprints stay at $H_\text{ref}$. | IP-LV-9 | [terrain_lod_rendering.md §1 / Integration / OQ-LR-1](../design/terrain_lod_rendering.md), [screen_space_lod_selection.md](../algorithms/screen_space_lod_selection.md) |
...
policy of [terrain_lod_rendering.md](../design/terrain_lod_rendering.md). All the design questions are now resolved — OQ-LS-22 → per-LOD footprints,
[terrain_lod_rendering.md OQ-LR-2](../design/terrain_lod_rendering.md) → $\tau=1$ px, $\gamma=0.25$, and
OQ-LR-1 → runtime bands with footprints baked at $H_\text{ref}=1080$ — so the remaining chain is fully
unblocked: **IP-LV-9 → (IP-LV-5, IP-LV-10) → IP-LV-6**. IP-LV-9 makes the chunk grid per-LOD; IP-LV-5
(per-LOD footprint tiling + SSE thresholds in the build) and IP-LV-10 (runtime SSE visibility bands +
streaming radii in the viewer) both depend on it; IP-LV-6 validates culling, the per-LOD
transition-alignment gate, and runtime cost.
