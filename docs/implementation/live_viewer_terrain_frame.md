...
| IP-LV-5 | ready (after IP-LV-8) | **Uniform small tile footprint, region-wide.** Set [`_LOD_CELL_SIDE_DEG`](../../python/tools/terrain/build_terrain.py) to a single small footprint (start ≈ 300–500 m) for **all** LODs, and export every LOD across the whole parameterized terrain region (so each LOD covers the region, LOD = vertex density within the fixed footprint). Confine small-tile generation to the specified region (do not re-tile past the dataset boundary). Re-tile `small_uas_ksba_flight`. **Ordering:** the re-tile must emit the streamable chunk format (IP-LV-8) — a monolithic GLB would have to be regenerated — so IP-LV-8 precedes this. | IP-LV-4, IP-LV-8 | [live_sim_view.md OQ-LS-18, OQ-LS-20](../design/live_sim_view.md) |
...
| IP-LV-7 | ready (OQ-LS-21 resolved) | **Tile-residency / streaming manager.** Godot `visibility_range` culls *rendering* only — every loaded tile's mesh + texture stays resident, and thousands of high-detail L0 textures exceed the ~4 GB GPU budget (≈ 3,400 L0 tiles alone at 512² RGBA8 ≈ 3.4 GB). Manager pages fine-LOD tiles by aircraft proximity with **asymmetric hysteresis** (OQ-LS-21 Alternative 1): load within a generous `R_fetch`, free only beyond a larger `R_unload` (`R_unload > R_fetch`); over-fetch is cheap so fetch may be generous/mildly predictive, eviction stays conservative. Coarse LODs stay permanently resident as the far backdrop. | IP-LV-8 | [live_sim_view.md OQ-LS-21](../design/live_sim_view.md) |
| IP-LV-8 | ready (OQ-LS-20 resolved) | **Streamable asset format.** The current single monolithic GLB (loaded whole via `ResourceLoader.load()` + `instantiate()`) cannot be paged. Emit **per-region chunk files** addressed by **integer chunk coordinate** `(lod, chunk_x, chunk_y)` (OQ-LS-20 Alternative 2) plus a small top-level descriptor (region origin, chunk size in footprints, footprint, LOD count, coverage bounds) and a sparse-occupancy record — **no flat all-tiles manifest**, so the format has headroom for several-hundred-km regions (millions of tiles). Chunk size = integer number of footprints aligned to the OQ-LS-21 `R_fetch`. Changes: `export_gltf.py` / `build_terrain.py` (emit chunks + descriptor) and `TerrainLoader.gd` (coordinate-addressable chunk load). | — | [live_sim_view.md OQ-LS-20](../design/live_sim_view.md) |
...
**IP-LV-8 → IP-LV-7 → IP-LV-5 → IP-LV-6**. Both streaming design questions are now **resolved**
(OQ-LS-20 → per-region coordinate-addressable chunk files; OQ-LS-21 → radius-based paging with
asymmetric hysteresis), so IP-LV-8 and IP-LV-7 are ready to implement. IP-LV-8 (asset format) precedes
IP-LV-5 (re-tile) so the re-tile emits the streamable chunk format directly.
