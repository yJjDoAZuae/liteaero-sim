...
| **Smooth-dynamics audit (SD-1)** — sweep every **flight-physics** model (aerodynamics, load-factor allocation/command shaping, airspeed/dynamic-pressure fades, propulsion, guidance, control) for slope-discontinuity corners at interior operating points (hard `sat`/`clamp`/`min`/`max`/`abs`, airspeed/mode switches) and reconcile each to a smooth (≥ C¹, preferably the C² smootherstep) form, per the [smooth-dynamics standard](../guidelines/general.md#smooth-dynamics--no-slope-discontinuities). Frictional and contact physics (tires, brakes, suspension, gear contact) are exempt (discontinuous by nature). The audit (read-only findings) and the per-site reconciliation (separate, explicitly-instructed code work) are distinct phases. | All flight-physics dynamics models | Active — landing-gear flight-physics coupling audited (findings in [landing_gear.md §Smoothness Audit](../design/landing_gear.md#smoothness-audit--slope-discontinuity-findings)); aerodynamics / load-factor allocator, propulsion, guidance, and control still to audit |
